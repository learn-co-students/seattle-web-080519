...
======================
- Where to find the methods that are available.
- Reference for where to find and organization of the guides.
- https://apidock.com/rails/ActiveRecord/QueryMethods/where
- Scuttle.io -> converts SQL Code to Active Record Queries.
- Asif : How are we seeding out database
  - Dataset from our API
  - hash
  - you look through the keys and grab values
  - {
    key1: value1,
    key2: value2
  }
  key1 -> column for your model in you DB
  key2 -> another column

  - Using those values you grabbed to create an instance (.new)
  - Save that to the database (.save)
  - .create = .new + .save


- No longer have instance variables
-

  Example = X.create(city : hash[:locale], key2: hash[:key2])
